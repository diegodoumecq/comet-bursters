...
export class PhaserSandboxScene extends BaseGameScene {
  ...
  private spawnPoint: Vector = { x: WORLD.width * 0.5, y: WORLD.height * 0.5 };
  ...
  create(): void {
    ...
    this.spawnPoint = this.chooseSafeSpawn();
    this.playerBody = new PlayerBody(this, this.spawnPoint, this.player);
  ...
  private updatePlayer(action: ReturnType<ActionReader['read']>, now: number, deltaSeconds: number): void {
    this.player.updateAim(normalize(action.aim));
    const move = normalize(action.move);
    this.player.updateThrust(move, false);
    if (this.player.visible) {
      const motion = updatePlayerMotion({ body: this.playerBody, deltaSeconds, move, player: this.player, ship: this.ship, world: WORLD, wrap: false });
      if (motion.thrusting) this.spawnThrusterParticle(move, now, motion.thrustScale);
      applyPlanetGravity(this.player.velocity, this.player.position, this.planets, WORLD, deltaSeconds);
      this.playerBody.setVelocity(this.player.velocity);
      this.rebaseWorldAroundPlayer();
    }
    const weaponResult = updateWeapons({
      action: { firePrimary: action.firePrimary, fireSecondary: action.fireSecondary, playerActive: this.player.visible, timeDilation: action.timeDilation },
      deltaSeconds,
    ...
    if (weaponResult.recoil.x !== 0 || weaponResult.recoil.y !== 0) {
      this.playerBody.setVelocity({ x: this.player.velocity.x + weaponResult.recoil.x, y: this.player.velocity.y + weaponResult.recoil.y });
    }
    for (const projectile of weaponResult.projectiles) this.addProjectile(projectile);
    applyTractorBeam(this.player.position, this.player.lastAim, this.asteroids, this.asteroidBodies, weaponResult.tractorActive);
  ...
  private updateWorld(deltaMs: number, deltaSeconds: number): void {
    ...
    for (const projectile of this.projectiles) {
      if (projectile.kind !== 'blackHole') applyPlanetGravity(projectile.velocity, projectile.position, this.planets, WORLD, deltaSeconds);
      this.projectileBodies.get(projectile).setVelocity(projectile.velocity.x, projectile.velocity.y);
    }
    for (const projectile of updateProjectiles(this.projectiles, this.projectileBodies, deltaSeconds, WORLD, false)) this.removeProjectile(projectile);
    const fuel = updateFuelBlobs(this.fuelBlobs, this.player.position, this.player.visible && this.ship.fuel < MAX_FUEL, deltaSeconds, WORLD, false);
  ...
  private resolveCombat(now: number, shieldActive: boolean): void {
    ...
    updateBlackHoles(
      ...
      (projectile) => {
        if (projectile.absorbedFuel > 0) this.addFuelBlobs(spawnFuelBlobs(projectile.position, projectile.velocity, projectile.absorbedFuel));
  ...
  private resolvePlanetCollisions(): void {
    for (const asteroid of [...this.asteroids]) {
      if (this.collidesWithPlanet(asteroid.position, ASTEROIDS[asteroid.tier].collisionRadius)) this.destroyAsteroid(asteroid, false);
    }
    for (const projectile of [...this.projectiles]) {
      if (projectile.kind !== 'blackHole' && this.collidesWithPlanet(projectile.position, 6)) this.removeProjectile(projectile);
    }
    if (this.player.visible && this.collidesWithPlanet(this.player.position, 18)) this.killPlayer(this.time.now);
  }
  ...
  private killPlayer(now: number): void {
    ...
    for (const effect of createShipExplosion(this.player.position, this.player.velocity)) this.applyEffect(effect);
  ...
  private chooseSafeSpawn(): Vector {
    for (let attempt = 0; attempt < 60; attempt += 1) {
      const candidate = { x: WORLD.width * 0.5 + Phaser.Math.Between(-700, 700), y: WORLD.height * 0.5 + Phaser.Math.Between(-700, 700) };
  ...
  private rebaseWorldAroundPlayer(): void {
    const shift = {
      x: this.player.position.x < 0 ? WORLD.width : this.player.position.x > WORLD.width ? -WORLD.width : 0,
      y: this.player.position.y < 0 ? WORLD.height : this.player.position.y > WORLD.height ? -WORLD.height : 0,
    };
    ...
    this.spawnPoint.x += shift.x;
    this.spawnPoint.y += shift.y;
  }
  ...
  private spawnThrusterParticle(move: Vector, now: number, thrustScale: number): void {
    ...
    const emitter = { x: this.player.position.x + exhaustDirection.x * 30, y: this.player.position.y + exhaustDirection.y * 30 };
...
function createInitialAsteroids(): AsteroidEntity[] {
  ...
  for (let index = 0; index < INITIAL_ASTEROIDS; index += 1) {
    ...
    asteroids.push(createAsteroid(
      tier,
      { x: Phaser.Math.Between(200, WORLD.width - 200), y: Phaser.Math.Between(200, WORLD.height - 200) },
