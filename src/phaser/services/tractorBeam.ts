...
import type { AsteroidEntity, Vector } from '../model';
import { normalize } from './world';
...
export function applyTractorBeam(
  ...
  for (const asteroid of asteroids) {
    const dx = asteroid.body.x - origin.x;
    const dy = asteroid.body.y - origin.y;
    const distance = Math.hypot(dx, dy);
    const forward = dx * direction.x + dy * direction.y;
    const angle = distance > 0 ? Math.acos(Math.max(-1, Math.min(1, forward / distance))) : Infinity;
    const focusDistance = Math.hypot(focus.x - asteroid.body.x, focus.y - asteroid.body.y);
    ...
    if (affected && focusDistance > 0) {
      ...
      asteroid.body.applyForce(
        new Phaser.Math.Vector2(
          ((focus.x - asteroid.body.x) / focusDistance) * (0.018 / softened),
          ((focus.y - asteroid.body.y) / focusDistance) * (0.018 / softened),
        ),
      );
    }
  }
