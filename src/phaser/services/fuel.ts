import type { AsteroidTier, ProjectileKind, WeaponKind } from '../model';
import { FUEL_COST } from './weapons';
...
export function consumeTractorFuel(fuel: number, deltaSeconds: number, active: boolean): number {
  return active ? Math.max(0, fuel - FUEL_COST.tractor * deltaSeconds * 60) : fuel;
}
...
export function getFireMode(fuel: number, weapon: WeaponKind): FireMode | null {
  ...
  return fuel <= LOW_FUEL_THRESHOLD || fuel < FUEL_COST[weapon] ? null : 'normal';
}
...
export function spendWeaponFuel(fuel: number, weapon: ProjectileKind, mode: FireMode): number {
  ...
  return Math.max(0, fuel - FUEL_COST[weapon]);
}
