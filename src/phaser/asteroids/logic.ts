...
export function wrapAsteroid(asteroid: AsteroidEntity, runtime: AsteroidBodies, world: WorldSize): void {
...
export function splitAsteroid(asteroid: AsteroidEntity): AsteroidEntity[] {
  ...
  for (let i = 0; i < config.splitCount; i += 1) {
    ...
    children.push(createAsteroid(
      config.child,
      {
        x: asteroid.position.x + Math.cos(angle) * spawnDistance,
        y: asteroid.position.y + Math.sin(angle) * spawnDistance,
      },
