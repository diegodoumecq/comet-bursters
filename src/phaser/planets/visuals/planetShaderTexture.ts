...
export type PlanetShaderUniformSetter = (gl: WebGLRenderingContext, program: WebGLProgram) => void;
...
export function createPlanetShaderTexture(
  scene: Phaser.Scene,
  textureKey: string,
  fragmentShader: string,
  textureSize: number,
  setUniforms: PlanetShaderUniformSetter,
): boolean {
...
export function getPlanetShaderSeed(planet: {
  altitudeVariations: number[];
  id: number;
  radius: number;
}): number {
  const altitudeSeed = planet.altitudeVariations.reduce(
    (total, value, index) => total + value * (index + 1) * 13.17,
    0,
  );
  return planet.id * 19.31 + planet.radius * 0.071 + altitudeSeed;
}
