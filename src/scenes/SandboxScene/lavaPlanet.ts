...
export function drawLavaPlanet(planet: Planet, ctx: CanvasRenderingContext2D): void {
  ...
  shellLightOverlay.addColorStop(0, withAlpha(tintColor(planet.color, style.shellInnerTint), 0.14));
  shellLightOverlay.addColorStop(
    Math.max(0.18, style.shellMidStop - 0.08),
    withAlpha(tintColor(planet.color, style.shellMidTint), 0.06),
  );
  shellLightOverlay.addColorStop(style.shellBaseStop, withAlpha(planet.color, 0.02));
  shellLightOverlay.addColorStop(1, withAlpha(tintColor(planet.color, style.shellOuterTint), 0.05));
